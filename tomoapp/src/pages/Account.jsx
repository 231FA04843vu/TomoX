...
function Account({ user, onUserUpdate }) {
  ...
  const saveNotificationPreferences = async (event) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Update failed");
      setProfileStatus({ type: "success", message: "Notification preferences updated" });
      if (onUserUpdate) onUserUpdate(data.user);
    } catch (error) {
      setProfileStatus({
        type: "error",
    ...
    <div className="account-container">
      ...
      <div className="account-grid">
        ...
        >
          ...
          <div className="tile-meta">Email preferences</div>
        </button>
              ...
              {activeModal === "notifications" && (
                <section className="account-panel">
                  <div className="panel-header">
                    ...
                    <span>Control what you receive</span>
                  </div>
                  <form onSubmit={saveNotificationPreferences} className="account-form">
                    <div className="notification-preferences-grid">
                      <label className="preference-card">
                        <div>
                          <strong>Email updates</strong>
                          <p>Order updates and support responses on email.</p>
                        </div>
                        <input
                          type="checkbox"
